...
//ALIENTEK STM32¿ª·¢°å
//2.8´ç/3.5´ç/4.3´ç/7´ç TFTÒº¾§Çı¶¯	  
//Ö§³ÖÇı¶¯ICĞÍºÅ°üÀ¨:ILI9341/NT35310/NT35510/SSD1963µÈ		    
//ÕıµãÔ­×Ó@ALIENTEK
...
{   
	regval=regval;		//Ê¹ÓÃ-O2ÓÅ»¯µÄÊ±ºò,±ØĞë²åÈëµÄÑÓÊ±
...
{	  
	data=data;			//Ê¹ÓÃ-O2ÓÅ»¯µÄÊ±ºò,±ØĞë²åÈëµÄÑÓÊ±
	LCD->LCD_RAM=data;		 
}
//¶ÁLCDÊı¾İ
//·µ»ØÖµ:¶Áµ½µÄÖµ
...
uint32_t LCD_ReadPoint(uint16_t x,uint16_t y)
{
	...
	LCD_SetCursor(x,y);	    
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X1963)LCD_WR_REG(0X2E);//9341/3510/1963 ·¢ËÍ¶ÁGRAMÖ¸Áî
	...
	//9341/NT35310/NT35510Òª·Ö2´Î¶Á³ö 
	opt_delay(2);	                                    
	b=LCD_RD_DATA(); 
...
{					   
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X1963)LCD_WR_REG(0X29);	//¿ªÆôÏÔÊ¾
	else if(lcddev.id==0X5510)LCD_WR_REG(0X2900);	//¿ªÆôÏÔÊ¾
}	 
//LCD¹Ø±ÕÏÔÊ¾
void LCD_DisplayOff(void)
{	   
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X1963)LCD_WR_REG(0X28);	//¹Ø±ÕÏÔÊ¾
	else if(lcddev.id==0X5510)LCD_WR_REG(0X2800);	//¹Ø±ÕÏÔÊ¾
}   
//ÉèÖÃ¹â±êÎ»ÖÃ(¶ÔRGBÆÁÎŞĞ§)
//Xpos:ºá×ø±ê
//Ypos:×İ×ø±ê
...
//ÉèÖÃLCDµÄ×Ô¶¯É¨Ãè·½Ïò(¶ÔRGBÆÁÎŞĞ§)
//×¢Òâ:ÆäËûº¯Êı¿ÉÄÜ»áÊÜµ½´Ëº¯ÊıÉèÖÃµÄÓ°Ïì(ÓÈÆäÊÇ9341),
...
void LCD_Scan_Dir(uint8_t dir)
{
	...
	uint16_t dirreg=0;
	uint16_t temp;  
	if((lcddev.dir==1&&lcddev.id!=0X1963)||(lcddev.dir==0&&lcddev.id==0X1963))//ºáÆÁÊ±£¬¶Ô1963²»¸Ä±äÉ¨Ãè·½Ïò£¡ÊúÆÁÊ±1963¸Ä±ä·½Ïò
	{			   
		switch(dir)//·½Ïò×ª»»
		{
			case 0:dir=6;break;
...
//ÉèÖÃ´°¿Ú(¶ÔRGBÆÁÎŞĞ§),²¢×Ô¶¯ÉèÖÃ»­µã×ø±êµ½´°¿Ú×óÉÏ½Ç(sx,sy).
...
//SRAMµ×²ãÇı¶¯£¬Ê±ÖÓÊ¹ÄÜ£¬Òı½Å·ÖÅä
//´Ëº¯Êı»á±»HAL_SRAM_Init()µ÷ÓÃ
//hsram:SRAM¾ä±ú
...
//³õÊ¼»¯lcd
//¸Ã³õÊ¼»¯º¯Êı¿ÉÒÔ³õÊ¼»¯¸÷ÖÖĞÍºÅµÄLCD(Ïê¼û±¾.cÎÄ¼ş×îÇ°ÃæµÄÃèÊö)
void LCD_Init(void)
{ 	  
	GPIO_InitTypeDef GPIO_Initure;
	...
	TFTSRAM_Handler.Init.DataAddressMux=FSMC_DATA_ADDRESS_MUX_DISABLE; 	//µØÖ·/Êı¾İÏß²»¸´ÓÃ
	TFTSRAM_Handler.Init.MemoryType=FSMC_MEMORY_TYPE_SRAM;   			//SRAM
	TFTSRAM_Handler.Init.MemoryDataWidth=FSMC_NORSRAM_MEM_BUS_WIDTH_16; //16Î»Êı¾İ¿í¶È
	TFTSRAM_Handler.Init.BurstAccessMode=FSMC_BURST_ACCESS_MODE_DISABLE; //ÊÇ·ñÊ¹ÄÜÍ»·¢·ÃÎÊ,½ö¶ÔÍ¬²½Í»·¢´æ´¢Æ÷ÓĞĞ§,´Ë´¦Î´ÓÃµ½
	TFTSRAM_Handler.Init.WaitSignalPolarity=FSMC_WAIT_SIGNAL_POLARITY_LOW;//µÈ´ıĞÅºÅµÄ¼«ĞÔ,½öÔÚÍ»·¢Ä£Ê½·ÃÎÊÏÂÓĞÓÃ
	TFTSRAM_Handler.Init.WaitSignalActive=FSMC_WAIT_TIMING_BEFORE_WS;   //´æ´¢Æ÷ÊÇÔÚµÈ´ıÖÜÆÚÖ®Ç°µÄÒ»¸öÊ±ÖÓÖÜÆÚ»¹ÊÇµÈ´ıÖÜÆÚÆÚ¼äÊ¹ÄÜNWAIT
	TFTSRAM_Handler.Init.WriteOperation=FSMC_WRITE_OPERATION_ENABLE;    //´æ´¢Æ÷Ğ´Ê¹ÄÜ
	TFTSRAM_Handler.Init.WaitSignal=FSMC_WAIT_SIGNAL_DISABLE;           //µÈ´ıÊ¹ÄÜÎ»,´Ë´¦Î´ÓÃµ½
	TFTSRAM_Handler.Init.ExtendedMode=FSMC_EXTENDED_MODE_ENABLE;        //¶ÁĞ´Ê¹ÓÃ²»Í¬µÄÊ±Ğò
	TFTSRAM_Handler.Init.AsynchronousWait=FSMC_ASYNCHRONOUS_WAIT_DISABLE;//ÊÇ·ñÊ¹ÄÜÍ¬²½´«ÊäÄ£Ê½ÏÂµÄµÈ´ıĞÅºÅ,´Ë´¦Î´ÓÃµ½
	TFTSRAM_Handler.Init.WriteBurst=FSMC_WRITE_BURST_DISABLE;           //½ûÖ¹Í»·¢Ğ´
	...
	FSMC_ReadWriteTim.DataSetupTime=60;				//Êı¾İ±£´æÊ±¼äÎª60¸öHCLK	=6*60=360ns
	FSMC_ReadWriteTim.AccessMode=FSMC_ACCESS_MODE_A;//Ä£Ê½A
	//FMCĞ´Ê±Ğò¿ØÖÆ¼Ä´æÆ÷
	FSMC_WriteTim.BusTurnAroundDuration=0;			//×ÜÏßÖÜ×ª½×¶Î³ÖĞøÊ±¼äÎª0£¬´Ë±äÁ¿²»¸³ÖµµÄ»°»áÄªÃûÆäÃîµÄ×Ô¶¯ĞŞ¸ÄÎª4¡£µ¼ÖÂ³ÌĞòÔËĞĞÕı³£
	FSMC_WriteTim.AddressSetupTime=9;          		//µØÖ·½¨Á¢Ê±¼ä£¨ADDSET£©Îª9¸öHCLK =54ns 
	FSMC_WriteTim.AddressHoldTime=0;
	FSMC_WriteTim.DataSetupTime=8;              	//Êı¾İ±£´æÊ±¼äÎª6ns*9¸öHCLK=54n
	...
	//³õÊ¼»¯Íê³ÉÒÔºó,ÌáËÙ
	if(lcddev.id==0X9341||lcddev.id==0X5310||lcddev.id==0X5510||lcddev.id==0X1963)//Èç¹ûÊÇÕâ¼¸¸öIC,ÔòÉèÖÃWRÊ±ĞòÎª×î¿ì
	{
		//ÖØĞÂÅäÖÃĞ´Ê±Ğò¿ØÖÆ¼Ä´æÆ÷µÄÊ±Ğò   	 							    
		FSMC_Bank1E->BWTR[6]&=~(0XF<<0);//µØÖ·½¨Á¢Ê±¼ä(ADDSET)ÇåÁã 	 
		FSMC_Bank1E->BWTR[6]&=~(0XF<<8);//Êı¾İ±£´æÊ±¼äÇåÁã
		FSMC_Bank1E->BWTR[6]|=3<<0;		//µØÖ·½¨Á¢Ê±¼ä(ADDSET)Îª3¸öHCLK =18ns  	 
		FSMC_Bank1E->BWTR[6]|=2<<8; 	//Êı¾İ±£´æÊ±¼ä(DATAST)Îª6ns*3¸öHCLK=18ns
	}
...
{  							  
	...
	{   
		...
		else if(size==32)temp=asc2_3216[num][t];	//µ÷ÓÃ3216×ÖÌå
...
void LCD_Show_Chinese(uint16_t x, uint16_t y, uint8_t num, uint8_t size)
{
    uint8_t temp, t1;
    uint16_t y0 = y;
    uint16_t bytes_per_char;   // æ¯ä¸ªæ±‰å­—æ‰€å å­—èŠ‚æ•°
    uint16_t t;
    uint8_t mode = 0;

    // è®¡ç®—æ±‰å­—ç‚¹é˜µå­—èŠ‚æ•°
    if(size == 16) bytes_per_char = 32;    // 16*16/8
    else if(size == 24) bytes_per_char = 72; // 24*24/8
    else if(size == 32) bytes_per_char = 128; // 32*32/8
    else return; // ä¸æ”¯æŒçš„å­—åº“å¤§å°

    for(t = 0; t < bytes_per_char; t++)
    {
        if(size == 16)      temp = chinese_1616[num][t];
        else if(size == 24) temp = chinese_2424[num][t];
        else if(size == 32) temp = chinese_3232[num][t];
        else return;
